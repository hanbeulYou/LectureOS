...
class SubtitleTranscriptIntakeServiceTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.service = SubtitleTranscriptIntakeService(
    ...
    def _plan(self, name="intake") -> SubtitleIntakeIdentityPlan:
    ...
    def _evaluate(self, readiness, **overrides):
        ...
        return self.service.evaluate_intake(**base)

    def test_ready_is_eligible_with_full_linkage(self) -> None:
        readiness = self._readiness(ready=True)
        prepared = self._evaluate(readiness)
        intake = prepared.intake
        self.assertIs(intake.outcome, SubtitleIntakeOutcome.ELIGIBLE)
        self.assertIs(intake.readiness_outcome, ReadinessOutcome.READY)
        self.assertEqual(intake.source_readiness_id, readiness.identity)
        self.assertEqual(intake.source_selection_id, TranscriptCurrentSelectionId("selection"))
        self.assertEqual(intake.source_decision_id, TranscriptReviewDecisionId("decision"))
        self.assertEqual(intake.review_item_id, ReviewItemId("item-0"))
        self.assertEqual(intake.candidate_reference_id, CandidateReferenceId("candidate-0"))
        self.assertEqual(intake.source_transcript_id, self.raw.identity)
        self.assertEqual(intake.source_revision_id, self.revision.identity)
        self.assertEqual(intake.source_media_id, self.media_id)
        self.assertEqual(intake.source_timeline_id, self.timeline_id)
        self.assertEqual(intake.validation_id, TranscriptValidationId("validation"))
        self.assertEqual(intake.run_id, self.run_id)
        self.assertEqual(prepared.intake_result.kind, "subtitle_transcript_intake")
        self.assertEqual(
            prepared.intake_result.upstream_results, (readiness.domain_result_id,)
        )

    def test_not_ready_is_not_eligible(self) -> None:
        readiness = self._readiness(name="not-ready", ready=False)
        prepared = self._evaluate(readiness)
        self.assertIs(prepared.intake.outcome, SubtitleIntakeOutcome.NOT_ELIGIBLE)
