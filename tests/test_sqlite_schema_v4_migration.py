...
class SQLiteSchemaVersionFourTests(unittest.TestCase):
    ...
    def test_new_database_includes_complete_frozen_v4(self) -> None:
        ...
        try:
            ...
            self.assertEqual(connection.execute("SELECT version FROM schema_metadata").fetchone(), (49,))
            self.assertTrue(V4_TABLES.issubset(table_names(connection)))
            self.assertEqual(sqlite_lifecycle.validate_sqlite_connection(connection), 49)
        finally:
            connection.close()
