...
V36_TABLES = {"corrected_revision_generations"}
...
class SQLiteSchemaVersionThirtySixTests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_v36_remains_a_supported_version(self) -> None:
        self.assertIn(36, sqlite_lifecycle._SUPPORTED_SCHEMA_VERSIONS)
        self.assertLessEqual(36, SQLITE_SCHEMA_VERSION)

    def test_fresh_database_initializes_with_v36_tables(self) -> None:
        connection = initialize_sqlite_database(self.database_path)
        try:
            self.assertTrue(V36_TABLES.issubset(table_names(connection)))
            self.assertEqual(
    ...
    def test_migrates_v35_to_v36_preserving_existing_rows(self) -> None:
        create_legacy_database(self.database_path, 35)
    ...
    def test_v36_no_op_migration_is_allowed(self) -> None:
        create_legacy_database(self.database_path, 35)
        migrate_sqlite_database(self.database_path, 36)
        migrate_sqlite_database(self.database_path, 36)
        connection = open_sqlite_database(self.database_path)
    ...
    def test_direct_v34_to_v36_is_rejected(self) -> None:
    ...
    def test_repository_rejects_pre_v36_schema(self) -> None:
        create_legacy_database(self.database_path, 35)
        connection = open_sqlite_database(self.database_path)
        try:
            with self.assertRaises(Exception):
                SQLiteCorrectedRevisionGenerationRepository(connection)
        finally:
            connection.close()

    def test_every_released_version_chains_to_v36_preserving_data(self) -> None:
