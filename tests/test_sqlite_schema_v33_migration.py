...
V33_TABLES = {"current_raw_transcript_selections"}
...
class SQLiteSchemaVersionThirtyThreeTests(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_v33_remains_a_supported_version(self) -> None:
        self.assertIn(33, sqlite_lifecycle._SUPPORTED_SCHEMA_VERSIONS)
        self.assertLessEqual(33, SQLITE_SCHEMA_VERSION)

    def test_fresh_database_initializes_with_v33_tables(self) -> None:
        connection = initialize_sqlite_database(self.database_path)
        try:
            self.assertTrue(V33_TABLES.issubset(table_names(connection)))
            self.assertEqual(
    ...
    def test_migrates_v32_to_v33_preserving_existing_rows(self) -> None:
    ...
    def test_v33_no_op_migration_is_allowed(self) -> None:
        create_legacy_database(self.database_path, 32)
        migrate_sqlite_database(self.database_path, 33)
        migrate_sqlite_database(self.database_path, 33)
        connection = open_sqlite_database(self.database_path)
    ...
    def test_direct_v31_to_v33_is_rejected(self) -> None:
    ...
    def test_repository_rejects_pre_v33_schema(self) -> None:
        create_legacy_database(self.database_path, 32)
        connection = open_sqlite_database(self.database_path)
        try:
    ...
    def test_every_released_version_chains_to_v33_preserving_data(self) -> None:
