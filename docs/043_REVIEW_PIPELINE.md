...
**`044 §26`과의 관계 (Confirmed, PATCH-0038):** AH-9의 **authority 파생은 이 note로 변경되지 않는다** — 이력을 가진 actor가 둘 이상이면 현재 유효한 판단을 파생하지 않고 `§3.12` Review Conflict로 표시한다는 규정도, 자동 authority 순위 도입 금지도, `§15.3`을 답하지 않는다는 유보도 그대로다. `044 §26`(CD-1…CD-11)이 확정한 것은 그 Conflict가 **Export Admission에 미치는 downstream 동작**뿐이다: 그 Candidate는 `044 §23` EA-4의 귀결로 Export membership에 기여하지 않되 그 사실이 Source Timeline 전체의 export를 막지 않으며, Conflict의 존재는 admission result의 **필수 disclosure**로 공개된다. `044 §26`은 AH-9의 결과를 **소비할 뿐 거기에 아무것도 더하지 않으며**, 특히 actor 간 우선순위·최신성·역할 서열을 도입하지 않는다. **withdrawal, revocation, 그리고 다중 actor 사이의 authority 해석은 `§15.3`·`§15.4`의 deferred 상태 그대로이며**, 그 결과 한 Candidate에 두 actor의 이력이 생기면 현행 계약으로는 Conflict가 해소되지 않을 수 있다 — 그 해소는 별도의 승인된 결정에 속한다.
