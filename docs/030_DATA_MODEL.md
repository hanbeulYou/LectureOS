...
Edit-Pipeline의 durable `ReviewDecision`과 `ApprovedEditDecision`의 canonical 기록 계약은 `043_REVIEW_PIPELINE.md §7.4`(`patches/PATCH-0014`)에서 확정되었다: 둘 다 durable·immutable·insert-only·identity-owning·provenance-bearing 기록으로, `ReviewDecision`은 정확히 하나의 `EditCandidate`에 anchor되고 닫힌 `{accept, reject, modify}` decision kind를 가지며, accept/modify는 각각 하나의 `ApprovedEditDecision`을(reject는 0개) 만든다. `ApprovedEditDecision`은 승인된 range·Candidate Type/label·rationale의 self-contained 스냅샷을 소유하고 `ReviewDecision`·`EditCandidate`를 참조한다. status 전이·Review Session/History·reconciliation·revision·supersession·export format은 여전히 deferred다. 이 기록 계약(두 기록의 성질, 단일 `EditCandidate` anchor의 cardinality와 방향, 닫힌 decision kind, Approved 생성 규칙과 승인 스냅샷 소유, Modify ownership, status 필드 없음)은 generation-neutral하다. 다만 anchor 대상 Candidate record와 admission 전제(실행 provenance, Domain Result, identity 소유, ordinal)는 contract generation마다 다르며, 그 점은 `043 §7.5`(`patches/PATCH-0033`)가 확정한다. 사람의 판단이 번복될 때 무엇이 현재 유효한지 판정하는 **authority history와 current selection의 표현**도 generation마다 다르며, effective-transcript generation에 대해서는 `043 §7.6`(`patches/PATCH-0034`)이 확정한다 — 두 기록에 status·ordinal 컬럼을 추가하지 않는 별개의 append-only 이력이고, 현재 유효한 판단은 저장되지 않고 파생된다. 위 기록 계약의 generation-neutral한 성질은 그 표현 선택에 영향받지 않는다.

어떤 `ApprovedEditDecision`이 Export에 도달하는가, 그리고 어떤 조건 아래 도달하는가도 contract generation마다 다르다. effective-transcript generation에 대해서는 `044_EXPORT_PIPELINE.md §23`(`patches/PATCH-0035`)이 확정한다 — Export Assembly가 하나의 Source Timeline의 **모든 export 적격 승인 편집**을 모으고, 적격성은 **현재 유효한 판단의 승인 + 단일 actor + 연쇄 뿌리 standing `current`**로 파생되며, 그 사이에 별도의 선택 단계나 승인 단계는 존재하지 않는다. **Edit Pipeline에 Final Selection이라는 제품 개념은 존재하지 않으며**, `7.3 Final Subtitle`은 Subtitle Pipeline 고유의 승인 결과로 이와 무관하다. 위 기록 계약의 generation-neutral한 성질은 이 범위 결정에 영향받지 않는다.
...
이 세대 Artifact의 **구체 external format과 로컬 파일 materialization**은 `044_EXPORT_PIPELINE.md §25`(`patches/PATCH-0037`)가 확정한다 — LectureOS-native JSON 하나이며, legacy와 **구별되는 format identity**를 쓰고(payload shape가 다르고 두 표현은 대체 관계가 아니다), 직렬화는 결정적이고, 파일은 caller가 지정한 로컬 destination에 원자적으로 배치되며, payload도 파일도 데이터베이스에 저장되지 않는다. 파일은 Artifact의 identity가 아니다.
