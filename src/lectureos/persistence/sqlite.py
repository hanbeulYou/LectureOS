...
_V21_ADDITION_STATEMENTS = (
    """CREATE TABLE subtitle_srt_artifacts (
    identity TEXT PRIMARY KEY,
    domain_result_id TEXT NOT NULL,
    source_approved_document_id TEXT NOT NULL,
    format TEXT NOT NULL CHECK (format IN ('srt')),
    ...
    CHECK ((cue_count = 0 AND payload = '') OR (cue_count > 0 AND length(payload) > 0)),
    CHECK ((sequence = 0 AND previous_artifact_id IS NULL) OR sequence > 0)
)""",
)

_V22_ADDITION_STATEMENTS = (
    """CREATE TABLE subtitle_srt_materializations (
    identity TEXT PRIMARY KEY,
    domain_result_id TEXT NOT NULL,
    source_artifact_id TEXT NOT NULL,
    storage_kind TEXT NOT NULL CHECK (storage_kind IN ('local_file')),
    ...
    sequence INTEGER NOT NULL CHECK (sequence >= 0),
    reason TEXT NOT NULL CHECK (length(trim(reason)) > 0),
    previous_materialization_id TEXT,
    CHECK ((sequence = 0 AND previous_materialization_id IS NULL) OR sequence > 0)
)""",
    """CREATE TABLE subtitle_srt_materialization_outcomes (
    subtitle_srt_materialization_id TEXT PRIMARY KEY,
    state TEXT NOT NULL CHECK (state IN ('materialized', 'failed')),
    byte_length INTEGER,
    failure_reason TEXT,
    CHECK ((state = 'materialized' AND byte_length IS NOT NULL AND byte_length >= 0
            AND failure_reason IS NULL)
           OR (state = 'failed' AND failure_reason IS NOT NULL
               AND length(trim(failure_reason)) > 0 AND byte_length IS NULL)),
    FOREIGN KEY (subtitle_srt_materialization_id)
        REFERENCES subtitle_srt_materializations(identity) ON DELETE CASCADE
)""",
)

_V23_ADDITION_STATEMENTS = (
    """CREATE TABLE eligible_analysis_inputs (
    identity TEXT PRIMARY KEY,
    domain_result_id TEXT NOT NULL,
    source_readiness_id TEXT NOT NULL,
    readiness_outcome TEXT NOT NULL CHECK (readiness_outcome IN ('ready', 'not_ready')),
    ...
    sequence INTEGER NOT NULL CHECK (sequence >= 0),
    reason TEXT NOT NULL CHECK (length(trim(reason)) > 0),
    previous_input_id TEXT,
    CHECK ((readiness_outcome = 'ready' AND eligibility = 'eligible')
           OR (readiness_outcome = 'not_ready' AND eligibility = 'not_eligible')),
    CHECK ((sequence = 0 AND previous_input_id IS NULL) OR sequence > 0)
)""",
)
...
_V27_ADDITION_STATEMENTS = (
    """CREATE TABLE edit_review_decisions (
    identity TEXT PRIMARY KEY,
    domain_result_id TEXT NOT NULL,
    source_candidate_id TEXT NOT NULL,
    decision_kind TEXT NOT NULL CHECK (decision_kind IN ('accept', 'reject', 'modify')),
    ...
    """CREATE TABLE approved_edit_decisions (
    identity TEXT PRIMARY KEY,
    domain_result_id TEXT NOT NULL,
    source_decision_id TEXT NOT NULL UNIQUE,
    source_candidate_id TEXT NOT NULL,
    decision_kind TEXT NOT NULL CHECK (decision_kind IN ('accept', 'modify')),
    approved_range_start REAL NOT NULL,
    approved_range_end REAL NOT NULL,
    approved_candidate_type TEXT NOT NULL CHECK (length(trim(approved_candidate_type)) > 0),
    ...
    unit_execution_id TEXT NOT NULL,
    sequence INTEGER NOT NULL CHECK (sequence >= 0),
    CHECK (approved_range_start >= 0 AND approved_range_end >= approved_range_start),
    FOREIGN KEY (source_decision_id) REFERENCES edit_review_decisions(identity) ON DELETE CASCADE
)""",
)

_V28_ADDITION_STATEMENTS = (
    """CREATE TABLE approved_edit_export_representations (
    identity TEXT PRIMARY KEY,
    domain_result_id TEXT NOT NULL,
    source_approved_decision_id TEXT NOT NULL,
    source_review_decision_id TEXT NOT NULL,
    source_candidate_id TEXT NOT NULL,
    decision_kind TEXT NOT NULL CHECK (decision_kind IN ('accept', 'modify')),
    approved_range_start REAL NOT NULL,
    ...
    sequence INTEGER NOT NULL CHECK (sequence >= 0),
    CHECK (approved_range_start >= 0 AND approved_range_end >= approved_range_start),
    FOREIGN KEY (source_approved_decision_id)
        REFERENCES approved_edit_decisions(identity) ON DELETE CASCADE
)""",
)
...
_V40_ADDITION_STATEMENTS = (
    """CREATE TABLE subtitle_effective_review_subjects (
    identity TEXT PRIMARY KEY,
    candidate_id TEXT NOT NULL,
    candidate_graph_fingerprint TEXT NOT NULL
        CHECK (length(candidate_graph_fingerprint) = 64),
    preparation_kind TEXT NOT NULL CHECK (length(trim(preparation_kind)) > 0),
    preparation_version INTEGER NOT NULL CHECK (preparation_version >= 1),
    preparation_key TEXT NOT NULL CHECK (length(trim(preparation_key)) > 0),
    UNIQUE (preparation_key),
    UNIQUE (candidate_id, preparation_kind, preparation_version),
    FOREIGN KEY (candidate_id) REFERENCES subtitle_effective_candidates(identity)
)""",
)
...
_V41_ADDITION_STATEMENTS = (
    ...
    previous_decision_id TEXT,
    rationale TEXT CHECK (rationale IS NULL OR length(trim(rationale)) > 0),
    UNIQUE (review_subject_id, sequence),
    CHECK ((sequence = 0 AND previous_decision_id IS NULL) OR
           (sequence > 0 AND previous_decision_id IS NOT NULL)),
    CHECK (previous_decision_id IS NULL OR previous_decision_id <> identity),
    FOREIGN KEY (review_subject_id)
        REFERENCES subtitle_effective_review_subjects(identity)
)""",
)

_V42_ADDITION_STATEMENTS = (
    """CREATE TABLE subtitle_effective_final_selections (
    identity TEXT PRIMARY KEY,
    transcript_source_intake_id TEXT NOT NULL,
    candidate_id TEXT NOT NULL,
    review_subject_id TEXT NOT NULL,
    supporting_decision_id TEXT NOT NULL,
    ...
    FOREIGN KEY (review_subject_id)
        REFERENCES subtitle_effective_review_subjects(identity),
    FOREIGN KEY (supporting_decision_id)
        REFERENCES subtitle_effective_review_decisions(identity)
)""",
)
...
_V44_ADDITION_STATEMENTS = (
    """CREATE TABLE subtitle_effective_srt_materializations (
    identity TEXT PRIMARY KEY,
    ...
    sequence INTEGER NOT NULL CHECK (sequence >= 0),
    previous_materialization_id TEXT,
    UNIQUE (artifact_id, relative_location, sequence),
    CHECK ((sequence = 0 AND previous_materialization_id IS NULL) OR
           (sequence > 0 AND previous_materialization_id IS NOT NULL)),
    CHECK (previous_materialization_id IS NULL OR previous_materialization_id <> identity),
    FOREIGN KEY (artifact_id) REFERENCES subtitle_effective_srt_artifacts(identity)
)""",
    """CREATE TABLE subtitle_effective_srt_materialization_outcomes (
...
_V46_ADDITION_STATEMENTS = (
    ...
    rationale TEXT CHECK (rationale IS NULL OR length(trim(rationale)) > 0),
    UNIQUE (transcript_source_intake_id, sequence),
    CHECK ((kind = 'publish' AND target_delivery_id IS NOT NULL
            AND target_artifact_id IS NOT NULL)
        OR (kind = 'withdraw' AND target_delivery_id IS NULL
            AND target_artifact_id IS NULL)),
    CHECK ((sequence = 0 AND previous_publication_id IS NULL) OR
           (sequence > 0 AND previous_publication_id IS NOT NULL)),
    CHECK (previous_publication_id IS NULL OR previous_publication_id <> identity),
    FOREIGN KEY (transcript_source_intake_id)
        REFERENCES transcript_source_intakes(identity),
    FOREIGN KEY (target_delivery_id)
        REFERENCES subtitle_effective_srt_delivery_intents(identity),
    FOREIGN KEY (target_artifact_id)
        REFERENCES subtitle_effective_srt_artifacts(identity)
)""",
)

_V47_ADDITION_STATEMENTS = (
    """CREATE TABLE lecture_analysis_input_admissions (
    identity TEXT PRIMARY KEY,
    transcript_source_intake_id TEXT NOT NULL,
    source_media_id TEXT NOT NULL,
    corrected_revision_id TEXT NOT NULL,
    parent_raw_transcript_id TEXT NOT NULL,
    raw_selection_id TEXT NOT NULL,
    corrected_selection_id TEXT NOT NULL,
    content_fingerprint TEXT NOT NULL CHECK (length(content_fingerprint) = 64),
    segment_count INTEGER NOT NULL CHECK (segment_count > 0),
    admission_contract_version INTEGER NOT NULL
        CHECK (admission_contract_version = 1),
    UNIQUE (transcript_source_intake_id, corrected_revision_id),
    FOREIGN KEY (transcript_source_intake_id)
        REFERENCES transcript_source_intakes(identity),
    FOREIGN KEY (source_media_id) REFERENCES source_media(identity),
    FOREIGN KEY (corrected_revision_id)
...
_V48_ADDITION_STATEMENTS = (
    """CREATE TABLE lecture_analysis_findings (
    identity TEXT PRIMARY KEY,
    admission_id TEXT NOT NULL,
    finding_type TEXT NOT NULL CHECK (length(finding_type) > 0),
    evidence TEXT NOT NULL CHECK (length(trim(evidence)) > 0),
    confidence REAL
        CHECK (confidence IS NULL OR (confidence >= 0.0 AND confidence <= 1.0)),
    uncertainty REAL
        CHECK (uncertainty IS NULL OR (uncertainty >= 0.0 AND uncertainty <= 1.0)),
    range_start REAL CHECK (range_start IS NULL OR range_start >= 0.0),
    range_end REAL CHECK (range_end IS NULL OR range_end >= 0.0),
    finding_contract_version INTEGER NOT NULL
        CHECK (finding_contract_version = 1),
    CHECK ((range_start IS NULL) = (range_end IS NULL)),
    CHECK (range_start IS NULL OR range_start <= range_end),
    FOREIGN KEY (admission_id)
        REFERENCES lecture_analysis_input_admissions(identity)
)""",
)

_V49_ADDITION_STATEMENTS = (
    """CREATE TABLE lecture_analysis_segments (
    identity TEXT PRIMARY KEY,
    admission_id TEXT NOT NULL,
    sequence INTEGER NOT NULL CHECK (sequence >= 0),
    range_start REAL NOT NULL CHECK (range_start >= 0.0),
    range_end REAL NOT NULL CHECK (range_end >= 0.0),
    segment_contract_version INTEGER NOT NULL
        CHECK (segment_contract_version = 1),
...
_V52_ADDITION_STATEMENTS = (
    ...
    position_contract_version INTEGER NOT NULL
        CHECK (position_contract_version = 1),
    UNIQUE (candidate_id, actor, sequence),
    CHECK ((sequence = 0 AND previous_position_id IS NULL) OR
           (sequence > 0 AND previous_position_id IS NOT NULL)),
    CHECK (previous_position_id IS NULL OR previous_position_id <> identity),
    FOREIGN KEY (candidate_id) REFERENCES lecture_analysis_edit_candidates(identity),
    FOREIGN KEY (review_decision_id) REFERENCES lecture_review_decisions(identity),
    FOREIGN KEY (previous_position_id) REFERENCES lecture_review_authority_positions(identity)
)""",
)
...
_V21_EXPECTED_COLUMNS = {
    **_V20_EXPECTED_COLUMNS,
    "subtitle_srt_artifacts": (
        ("identity", "TEXT", 0, 1),
        ("domain_result_id", "TEXT", 1, 0),
        ("source_approved_document_id", "TEXT", 1, 0),
        ("format", "TEXT", 1, 0),
        ("payload", "TEXT", 1, 0),
        ...
        ("sequence", "INTEGER", 1, 0),
        ("reason", "TEXT", 1, 0),
        ("previous_artifact_id", "TEXT", 0, 0),
    ),
}

_V22_EXPECTED_COLUMNS = {
    **_V21_EXPECTED_COLUMNS,
    "subtitle_srt_materializations": (
        ("identity", "TEXT", 0, 1),
        ("domain_result_id", "TEXT", 1, 0),
        ("source_artifact_id", "TEXT", 1, 0),
        ...
        ("sequence", "INTEGER", 1, 0),
        ("reason", "TEXT", 1, 0),
        ("previous_materialization_id", "TEXT", 0, 0),
    ),
    "subtitle_srt_materialization_outcomes": (
        ("subtitle_srt_materialization_id", "TEXT", 0, 1),
        ("state", "TEXT", 1, 0),
...
_V27_EXPECTED_COLUMNS = {
    **_V26_EXPECTED_COLUMNS,
    ...
    "approved_edit_decisions": (
        ...
        ("approved_candidate_type", "TEXT", 1, 0),
...
_V28_EXPECTED_COLUMNS = {
    **_V27_EXPECTED_COLUMNS,
    "approved_edit_export_representations": (
        ("identity", "TEXT", 0, 1),
        ("domain_result_id", "TEXT", 1, 0),
        ("source_approved_decision_id", "TEXT", 1, 0),
        ("source_review_decision_id", "TEXT", 1, 0),
        ("source_candidate_id", "TEXT", 1, 0),
        ("decision_kind", "TEXT", 1, 0),
        ("approved_range_start", "REAL", 1, 0),
...
_V30_EXPECTED_COLUMNS = {
    **_V29_EXPECTED_COLUMNS,
    "source_media": (
        ("identity", "TEXT", 0, 1),
        ("fingerprint_algorithm", "TEXT", 1, 0),
        ("fingerprint_digest", "TEXT", 1, 0),
        ("byte_length", "INTEGER", 1, 0),
        ("observed_source_path", "TEXT", 1, 0),
    ),
}

_V31_EXPECTED_COLUMNS = {
    **_V30_EXPECTED_COLUMNS,
...
_V32_EXPECTED_COLUMNS = {
    **_V31_EXPECTED_COLUMNS,
    "provider_transcript_admissions": (
        ...
        ("provider_reference", "TEXT", 1, 0),
        ("provider_model", "TEXT", 0, 0),
        ("declared_language", "TEXT", 0, 0),
        ("provider_result_ref", "TEXT", 1, 0),
...
_V40_EXPECTED_COLUMNS = {
    **_V39_EXPECTED_COLUMNS,
    "subtitle_effective_review_subjects": (
        ("identity", "TEXT", 0, 1),
        ("candidate_id", "TEXT", 1, 0),
        ("candidate_graph_fingerprint", "TEXT", 1, 0),
        ("preparation_kind", "TEXT", 1, 0),
        ("preparation_version", "INTEGER", 1, 0),
        ("preparation_key", "TEXT", 1, 0),
    ),
}

_V41_EXPECTED_COLUMNS = {
    **_V40_EXPECTED_COLUMNS,
    "subtitle_effective_review_decisions": (
        ("identity", "TEXT", 0, 1),
        ("review_subject_id", "TEXT", 1, 0),
        ("kind", "TEXT", 1, 0),
        ("reviewer", "TEXT", 1, 0),
        ("sequence", "INTEGER", 1, 0),
        ("content_fingerprint", "TEXT", 1, 0),
...
_V42_EXPECTED_COLUMNS = {
    **_V41_EXPECTED_COLUMNS,
    "subtitle_effective_final_selections": (
        ...
        ("candidate_id", "TEXT", 1, 0),
        ("review_subject_id", "TEXT", 1, 0),
        ("supporting_decision_id", "TEXT", 1, 0),
        ("selector", "TEXT", 1, 0),
        ("sequence", "INTEGER", 1, 0),
        ("content_fingerprint", "TEXT", 1, 0),
        ("previous_selection_id", "TEXT", 0, 0),
...
_V46_EXPECTED_COLUMNS = {
    **_V45_EXPECTED_COLUMNS,
    "subtitle_effective_srt_publications": (
        ("identity", "TEXT", 0, 1),
        ("transcript_source_intake_id", "TEXT", 1, 0),
        ("kind", "TEXT", 1, 0),
        ("target_delivery_id", "TEXT", 0, 0),
        ("target_artifact_id", "TEXT", 0, 0),
        ("publisher", "TEXT", 1, 0),
        ("sequence", "INTEGER", 1, 0),
        ("content_fingerprint", "TEXT", 1, 0),
...
_V47_EXPECTED_COLUMNS = {
    **_V46_EXPECTED_COLUMNS,
    "lecture_analysis_input_admissions": (
        ...
        ("parent_raw_transcript_id", "TEXT", 1, 0),
        ("raw_selection_id", "TEXT", 1, 0),
        ("corrected_selection_id", "TEXT", 1, 0),
        ("content_fingerprint", "TEXT", 1, 0),
        ("segment_count", "INTEGER", 1, 0),
        ("admission_contract_version", "INTEGER", 1, 0),
    ),
}

_V48_EXPECTED_COLUMNS = {
    **_V47_EXPECTED_COLUMNS,
    "lecture_analysis_findings": (
        ...
        ("range_start", "REAL", 0, 0),
        ("range_end", "REAL", 0, 0),
        ("finding_contract_version", "INTEGER", 1, 0),
    ),
}
...
_V49_EXPECTED_COLUMNS = {
    **_V48_EXPECTED_COLUMNS,
    "lecture_analysis_segments": (
        ...
        ("range_start", "REAL", 1, 0),
        ("range_end", "REAL", 1, 0),
        ("segment_contract_version", "INTEGER", 1, 0),
    ),
}
...
_V52_EXPECTED_COLUMNS = {
    **_V51_EXPECTED_COLUMNS,
    "lecture_review_authority_positions": (
        ...
        ("sequence", "INTEGER", 1, 0),
        ("review_decision_id", "TEXT", 1, 0),
        ("previous_position_id", "TEXT", 0, 0),
        ("position_contract_version", "INTEGER", 1, 0),
    ),
}
...
def migrate_sqlite_database(
    ...
    try:
        ...
        if current_version == 19 and target_version == 20:
            ...
            return
        if current_version == 20 and target_version == 21:
            _migrate_v20_to_v21(connection)
            return
        if current_version == 21 and target_version == 22:
            _migrate_v21_to_v22(connection)
            return
        ...
        if current_version == 23 and target_version == 24:
            ...
            return
        if current_version == 24 and target_version == 25:
            _migrate_v24_to_v25(connection)
            return
        if current_version == 25 and target_version == 26:
            _migrate_v25_to_v26(connection)
            return
        if current_version == 26 and target_version == 27:
            _migrate_v26_to_v27(connection)
            return
        ...
        if current_version == 28 and target_version == 29:
            ...
            return
        if current_version == 29 and target_version == 30:
            _migrate_v29_to_v30(connection)
            return
        if current_version == 30 and target_version == 31:
            _migrate_v30_to_v31(connection)
            return
        if current_version == 31 and target_version == 32:
            _migrate_v31_to_v32(connection)
            return
        ...
        if current_version == 33 and target_version == 34:
            ...
            return
        if current_version == 34 and target_version == 35:
            _migrate_v34_to_v35(connection)
            return
        if current_version == 35 and target_version == 36:
            _migrate_v35_to_v36(connection)
            return
        ...
        if current_version == 38 and target_version == 39:
            ...
            return
        if current_version == 39 and target_version == 40:
            _migrate_v39_to_v40(connection)
            return
        if current_version == 40 and target_version == 41:
            _migrate_v40_to_v41(connection)
            return
        ...
        if current_version == 44 and target_version == 45:
            ...
            return
        if current_version == 45 and target_version == 46:
            _migrate_v45_to_v46(connection)
            return
        if current_version == 46 and target_version == 47:
            _migrate_v46_to_v47(connection)
            return
        if current_version == 47 and target_version == 48:
            _migrate_v47_to_v48(connection)
            return
...
def _migrate_v20_to_v21(connection: sqlite3.Connection) -> None:
    try:
        connection.execute("BEGIN IMMEDIATE")
        for statement in _V21_ADDITION_STATEMENTS:
            connection.execute(statement)
        connection.execute(
            "UPDATE schema_metadata SET version = 21 WHERE singleton = 1"
        )
        _validate_initialized_connection(connection)
        _commit(connection)
    except PersistenceError:
...
def _migrate_v21_to_v22(connection: sqlite3.Connection) -> None:
    try:
        connection.execute("BEGIN IMMEDIATE")
        for statement in _V22_ADDITION_STATEMENTS:
            connection.execute(statement)
        connection.execute(
            "UPDATE schema_metadata SET version = 22 WHERE singleton = 1"
        )
        _validate_initialized_connection(connection)
        _commit(connection)
    except PersistenceError:
...
def _migrate_v22_to_v23(connection: sqlite3.Connection) -> None:
    try:
        connection.execute("BEGIN IMMEDIATE")
        for statement in _V23_ADDITION_STATEMENTS:
            connection.execute(statement)
        connection.execute(
            "UPDATE schema_metadata SET version = 23 WHERE singleton = 1"
        )
        _validate_initialized_connection(connection)
        _commit(connection)
    except PersistenceError:
...
def _migrate_v24_to_v25(connection: sqlite3.Connection) -> None:
    try:
        connection.execute("BEGIN IMMEDIATE")
        for statement in _V25_ADDITION_STATEMENTS:
            connection.execute(statement)
        connection.execute(
            "UPDATE schema_metadata SET version = 25 WHERE singleton = 1"
        )
        _validate_initialized_connection(connection)
        _commit(connection)
    except PersistenceError:
...
def _migrate_v25_to_v26(connection: sqlite3.Connection) -> None:
    try:
        connection.execute("BEGIN IMMEDIATE")
        for statement in _V26_ADDITION_STATEMENTS:
            connection.execute(statement)
        connection.execute(
            "UPDATE schema_metadata SET version = 26 WHERE singleton = 1"
        )
        _validate_initialized_connection(connection)
        _commit(connection)
    except PersistenceError:
...
def _migrate_v26_to_v27(connection: sqlite3.Connection) -> None:
    try:
        connection.execute("BEGIN IMMEDIATE")
        for statement in _V27_ADDITION_STATEMENTS:
            connection.execute(statement)
        connection.execute(
            "UPDATE schema_metadata SET version = 27 WHERE singleton = 1"
        )
        _validate_initialized_connection(connection)
        _commit(connection)
    except PersistenceError:
...
def _migrate_v27_to_v28(connection: sqlite3.Connection) -> None:
    try:
        connection.execute("BEGIN IMMEDIATE")
        for statement in _V28_ADDITION_STATEMENTS:
            connection.execute(statement)
        connection.execute(
            "UPDATE schema_metadata SET version = 28 WHERE singleton = 1"
        )
        _validate_initialized_connection(connection)
        _commit(connection)
    except PersistenceError:
...
def _migrate_v29_to_v30(connection: sqlite3.Connection) -> None:
    try:
        connection.execute("BEGIN IMMEDIATE")
        for statement in _V30_ADDITION_STATEMENTS:
            connection.execute(statement)
        connection.execute(
            "UPDATE schema_metadata SET version = 30 WHERE singleton = 1"
        )
        _validate_initialized_connection(connection)
        _commit(connection)
    except PersistenceError:
...
def _migrate_v30_to_v31(connection: sqlite3.Connection) -> None:
    try:
        connection.execute("BEGIN IMMEDIATE")
        for statement in _V31_ADDITION_STATEMENTS:
            connection.execute(statement)
        connection.execute(
            "UPDATE schema_metadata SET version = 31 WHERE singleton = 1"
        )
        _validate_initialized_connection(connection)
        _commit(connection)
    except PersistenceError:
...
def _migrate_v31_to_v32(connection: sqlite3.Connection) -> None:
    try:
        connection.execute("BEGIN IMMEDIATE")
        for statement in _V32_ADDITION_STATEMENTS:
            connection.execute(statement)
        connection.execute(
...
def _migrate_v32_to_v33(connection: sqlite3.Connection) -> None:
    try:
        connection.execute("BEGIN IMMEDIATE")
        for statement in _V33_ADDITION_STATEMENTS:
            connection.execute(statement)
        connection.execute(
            "UPDATE schema_metadata SET version = 33 WHERE singleton = 1"
        )
        _validate_initialized_connection(connection)
        _commit(connection)
    except PersistenceError:
...
def _migrate_v34_to_v35(connection: sqlite3.Connection) -> None:
    try:
        connection.execute("BEGIN IMMEDIATE")
        for statement in _V35_ADDITION_STATEMENTS:
            connection.execute(statement)
        connection.execute(
...
def _migrate_v35_to_v36(connection: sqlite3.Connection) -> None:
    try:
        connection.execute("BEGIN IMMEDIATE")
        for statement in _V36_ADDITION_STATEMENTS:
            connection.execute(statement)
        connection.execute(
            "UPDATE schema_metadata SET version = 36 WHERE singleton = 1"
        )
        _validate_initialized_connection(connection)
        _commit(connection)
    except PersistenceError:
...
def _migrate_v36_to_v37(connection: sqlite3.Connection) -> None:
    try:
        connection.execute("BEGIN IMMEDIATE")
        for statement in _V37_ADDITION_STATEMENTS:
            connection.execute(statement)
        connection.execute(
            "UPDATE schema_metadata SET version = 37 WHERE singleton = 1"
        )
        _validate_initialized_connection(connection)
        _commit(connection)
    except PersistenceError:
...
def _migrate_v51_to_v52(connection: sqlite3.Connection) -> None:
    try:
        connection.execute("BEGIN IMMEDIATE")
        for statement in _V52_ADDITION_STATEMENTS:
            connection.execute(statement)
        connection.execute(
            "UPDATE schema_metadata SET version = 52 WHERE singleton = 1"
        )
        _validate_initialized_connection(connection)
        _commit(connection)
    except PersistenceError:
...
def _migrate_v48_to_v49(connection: sqlite3.Connection) -> None:
    try:
        connection.execute("BEGIN IMMEDIATE")
        for statement in _V49_ADDITION_STATEMENTS:
            connection.execute(statement)
        connection.execute(
            "UPDATE schema_metadata SET version = 49 WHERE singleton = 1"
        )
        _validate_initialized_connection(connection)
        _commit(connection)
    except PersistenceError:
...
def _migrate_v47_to_v48(connection: sqlite3.Connection) -> None:
    try:
        connection.execute("BEGIN IMMEDIATE")
        for statement in _V48_ADDITION_STATEMENTS:
            connection.execute(statement)
        connection.execute(
            "UPDATE schema_metadata SET version = 48 WHERE singleton = 1"
        )
        _validate_initialized_connection(connection)
        _commit(connection)
    except PersistenceError:
...
def _migrate_v46_to_v47(connection: sqlite3.Connection) -> None:
    try:
        connection.execute("BEGIN IMMEDIATE")
        for statement in _V47_ADDITION_STATEMENTS:
            connection.execute(statement)
        connection.execute(
            "UPDATE schema_metadata SET version = 47 WHERE singleton = 1"
        )
        _validate_initialized_connection(connection)
        _commit(connection)
    except PersistenceError:
...
def _migrate_v45_to_v46(connection: sqlite3.Connection) -> None:
    try:
        connection.execute("BEGIN IMMEDIATE")
        for statement in _V46_ADDITION_STATEMENTS:
            connection.execute(statement)
        connection.execute(
            "UPDATE schema_metadata SET version = 46 WHERE singleton = 1"
        )
        _validate_initialized_connection(connection)
        _commit(connection)
    except PersistenceError:
...
def _migrate_v43_to_v44(connection: sqlite3.Connection) -> None:
    try:
        connection.execute("BEGIN IMMEDIATE")
        for statement in _V44_ADDITION_STATEMENTS:
            connection.execute(statement)
        connection.execute(
            "UPDATE schema_metadata SET version = 44 WHERE singleton = 1"
        )
        _validate_initialized_connection(connection)
        _commit(connection)
    except PersistenceError:
...
def _migrate_v41_to_v42(connection: sqlite3.Connection) -> None:
    try:
        connection.execute("BEGIN IMMEDIATE")
        for statement in _V42_ADDITION_STATEMENTS:
            connection.execute(statement)
        connection.execute(
            "UPDATE schema_metadata SET version = 42 WHERE singleton = 1"
        )
        _validate_initialized_connection(connection)
        _commit(connection)
    except PersistenceError:
...
def _migrate_v40_to_v41(connection: sqlite3.Connection) -> None:
    try:
        connection.execute("BEGIN IMMEDIATE")
        for statement in _V41_ADDITION_STATEMENTS:
            connection.execute(statement)
        connection.execute(
            "UPDATE schema_metadata SET version = 41 WHERE singleton = 1"
        )
        _validate_initialized_connection(connection)
        _commit(connection)
    except PersistenceError:
...
def _migrate_v39_to_v40(connection: sqlite3.Connection) -> None:
    try:
        connection.execute("BEGIN IMMEDIATE")
        for statement in _V40_ADDITION_STATEMENTS:
            connection.execute(statement)
        connection.execute(
            "UPDATE schema_metadata SET version = 40 WHERE singleton = 1"
        )
        _validate_initialized_connection(connection)
        _commit(connection)
    except PersistenceError:
