    ...
    def save(self, record: UnitExecution) -> None:
        ...
        try:
            ...
            self._insert_children(
                "unit_execution_inputs", "input_reference", record,
                tuple(item.value for item in record.input_references),
            )
            self._insert_children(
                "unit_execution_capabilities", "capability", record,
                tuple(item.value for item in record.capabilities),
            )
            self._insert_children(
                "unit_execution_plugins", "plugin_reference", record,
                tuple(item.value for item in record.plugins),
            )
            self._insert_children(
                "unit_execution_results", "domain_result_id", record,
                tuple(item.value for item in record.result_references),
            )
            self._insert_children(
                "unit_execution_failures", "failure_id", record,
                tuple(item.value for item in record.failure_references),
            )
            self._insert_children(
                "unit_execution_diagnostics", "diagnostic_id", record,
