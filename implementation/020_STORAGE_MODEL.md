...
- 재생성된 물리 표현이 이전 Artifact record와 같은 identity인지 새 생성 record인지는 Requires Validation이다.
...
구체적인 transaction, atomicity와 recovery 구현은 정의하지 않는다.
...
특정 filesystem 또는 object storage를 선택하지 않는다.
...
- Artifact 물리 파일이 손실된 경우 Artifact Record를 계속 유지해야 하는가?
- 재생성된 Artifact가 기존 Artifact와 같은 identity를 유지하는가?
...
- 물리 storage model과 schema
- transaction과 consistency 구현
